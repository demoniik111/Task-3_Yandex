...
self.addEventListener('install', event => {
    const promise = preCacheAllFavorites()
		...
		// Для инициации события "onactivate". Т.е. заставляет ServiceWorker стать активным
...
self.addEventListener('activate', event => {
    const promise = deleteObsoleteCaches()
        .then(() => {
			...
			// Чтобы установить этого работника как активного работника для всех клиентов, соответствующих сфере работника, и для иницииация события "oncontrollerchange" для клиентов. Так гласит сайт MDN. Сам понял это как передачу управления другому сервис-вокеру.
            self.clients.claim();
...
self.addEventListener('fetch', event => {
	...
	// В случае кириллического домена будет возвращён домен в punycode кодировке. Поэтому это нужно учесть.
	// Подойдёт ли такое построение ключа зависит от того, что вернет request.url. Если он вернёт корректные данные, то такое построение ключа подойдёт. 
	// Т.е. необходима следующая схема, чтобы origin вернул значения и не выдал ошибку: 'http' или 'https', затем '://', далее должен идти домен, далее может идти ':' и может идти порт
	// (порт по умолчанию, 80 и 443 соответственно, если явно указано).
...
function deleteObsoleteCaches() {
    return caches.keys()
        .then(names => {
			...
			// Судя по всему для удаления кешей
            return Promise.all(
